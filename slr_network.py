...
class WeightedResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc2(out)
        out = F.relu(out)
        out = (1.0 - alpha) * identity +alpha * out
        return out
...
class SLRModel(nn.Module):
    def __init__(
        ...
        self.conv2d = getattr(resnet, 'resnet18')()
        ...
        self.clip_study = gloss_level_alignment_loss


        self.cross_attn = CA(hidden_size, nhead=8, dropout=0.1)
        self.sa1 = SA(hidden_size, 8)
        self.sa2 = SA(hidden_size,8)
    ...
    def forward(self, x, len_x, label=None, label_lgt=None,gloss_index_batch=None,last_hidden_states = None):
        ...
        visual_gloss_feat = self.V2TAdapter(averaged_feats_tensor)


        visual_gloss_feat = self.sa1(visual_gloss_feat)
        pooled_text_feat = self.sa2(pooled_text_feat)



        visual_gloss_feat,  pooled_text_feat  = self.cross_attn(
            visual_gloss_feat,  # Q: [B, Lg, C]
            pooled_text_feat  # K,V: [B, Lg, C]
        )

        averaged_feats_tensor = visual_gloss_feat
